...
### II. Legacy Sources Are Frozen
All historical material in `_bmad-output/`, `tmp-document/`, and `archive/` is frozen reference material only. Those sources may be read for context, but they must not be indexed, promoted into active reading order, or treated as current authority.

### III. Clean SDD Only
The active documentation system is Spec Kit under `.specify/` and the current feature artifacts under `specs/`. Do not create or extend documentation indexes for frozen legacy sources. Keep the active SDD tree clean, current, and sourced from runtime truth.

### IV. Service Boundaries Are Explicit
The API Gateway is the canonical HTTP surface. The CV Parser is an asynchronous RabbitMQ worker. Notification is a scaffolded runtime shell with bootstrap, config validation, Prisma wiring, health checks, and messaging foundations, but its business delivery flows remain incomplete.

### V. Validate At The Boundaries
Validate HTTP payloads, queue messages, file metadata, config values, and external API responses at the edge. Use dedicated DTOs for nested shapes, explicit contract types, and strict mapping for inbound data. Do not use `any` for request, payload, or contract data when a concrete type can be defined.

### VI. Test The Contract, Not The Hunch
Non-trivial changes must be verified with the narrowest effective test at the touched boundary. Preserve the separation between unit, integration, and end-to-end tests. Cross-service behavior, auth, upload flow, queue flow, and contract changes require focused producer/consumer coverage.

### VII. Operate Safely And Transparently
...
- Treat `api-gateway/`, `cv-parser/`, and `notification/` as distinct parts with different maturity levels: implemented, partial, and scaffolded runtime.
- Use runtime entrypoints and current Spec Kit artifacts as authority; use frozen legacy sources only for one-time context recovery or comparison.
- Do not create indexes, catalogs, or cross-link graphs over frozen legacy sources.
...
- Review changes against the repository's actual runtime behavior, not only the planned architecture.
...
- The active documentation set is `.specify/` plus the feature artifacts under `specs/001-brownfield-context/` and the runtime references that are regenerated from code when needed.
...
**Version**: 1.3.0 | **Ratified**: 2026-05-05 | **Last Amended**: 2026-05-05
