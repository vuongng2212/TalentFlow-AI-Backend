...
The repository currently implements a two-runtime backend plus one partial runtime scaffold:

- **API Gateway:** NestJS HTTP entry point and orchestration layer
- **CV Parser:** Spring Boot queue consumer that parses uploaded CVs
- **Notification:** NestJS scaffold for future email, WebSocket, and notification-history features
...
| Notification service | Partial runtime scaffold                         | `notification/src/main.ts`, `notification/src/app.module.ts`                                                                  |
...
### 3. Notification remains partial

- The runtime scaffold already boots NestJS, exposes health checks, validates config, and serves a sample notification lookup route.
- The legacy planning docs still describe the eventual consumer set for `application.created`, `cv.parsed`, `cv.failed`, and `notification.send`.
- Email delivery, WebSocket fan-out, and RabbitMQ consumer flows are still incomplete in the current snapshot.
...
- **Notification**: runnable scaffold, but not feature-complete.
