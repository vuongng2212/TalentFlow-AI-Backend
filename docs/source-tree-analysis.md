...
The repository is organized as a multi-part backend with two runtime services and one planning-only service. Runtime truth comes from the service folders and infrastructure files.
...
├── notification/                 # Planning docs for the future notification service
...
### `notification/`
- **Purpose:** Planning-only documentation for a future service.
- **Contains:** `README.md` and `IMPLEMENTATION-PHASES.md`.
- **Status:** No runtime source tree exists in the current snapshot.
...
- **Notification:** documentation-first planning artifacts only.
...
- Notification should be treated as design material until runtime code appears.
